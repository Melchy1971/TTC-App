...
export const PinsManagement = () => {
  ...
  const loadMatches = async () => {
    let query = supabase
      .from("matches")
      .select("id, team, opponent, date, time")
      .order("date", { ascending: false });
                          ...
                          <SelectGroup key={team}>
                            ...
                            {matches.map((match) => (
                              <SelectItem key={match.id} value={match.id}>
                                {new Date(match.date).toLocaleDateString("de-DE")} - vs {match.opponent}
                              </SelectItem>
                            ))}
                          </SelectGroup>
                        ));
              ...
              {pins.length === 0 ? (
                ...
                pins.map((pin) => (
                  <TableRow key={pin.id}>
                    ...
                    <TableCell>{pin.match?.team || "-"}</TableCell>
                    <TableCell>{pin.match?.opponent || "-"}</TableCell>
                    <TableCell>
                      {editingPin === pin.id ? (
                        <Input
