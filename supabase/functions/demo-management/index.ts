...
async function resetCategory(supabase: any, category: string) {
  ...
  // Get first user ID to protect from deletion
  const { data: firstUserData } = await supabase.rpc('get_first_user_id')
  const firstUserId = firstUserData

  switch (category) {
    case 'members':
      // Delete all profiles except first user (admin)
      const { data: profiles } = await supabase
        .from('profiles')
        .select('user_id')
        .neq('user_id', '00000000-0000-0000-0000-000000000000')
        .neq('user_id', firstUserId)
      ...
      console.log(`Deleted ${profiles?.length || 0} members (first admin protected)`)
      break
...
async function resetAll(supabase: any) {
  ...
  // Delete ALL profiles and users including admin
  const { data: profiles } = await supabase
    .from('profiles')
    .select('user_id')
    .neq('user_id', '00000000-0000-0000-0000-000000000000')
  ...
  if (profiles && profiles.length > 0) {
    ...
    console.log(`Deleted ${profiles.length} users including admin`)
  }

  console.log('All data reset successfully - System zurück auf Auslieferungszustand')
}
